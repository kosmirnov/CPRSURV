...
#### summmary data for VOL,DTI,CSCORE,OCC_STAT, TAX

ordered_data <- data[order(FNMA$LOAN_ID, FNMA$START),]
...
SUM <- data.frame(SUM)
stargazer(SUM[c("YIELD.SPREAD","LTV","VOL","CSCORE","DTI","OCC_STAT","UNEMP")],nobs = FALSE, median=TRUE, iqr = TRUE)